...
from pytfeager.model.inception import InceptionNet
...
from pytfeager.config import inception_config as config
...
def main(_):
    ...
    # 初始化模型
    model = InceptionNet(num_layers=2,num_classes = FLAGS.num_classes)
    save_kwargs = {
        'model': model,
        'optimizer': optimizer,
    ...
    model_checkpoint = ModelCheckpoint(checkpoint_dir=checkpoint_dir, mode='min', monitor='val_loss',
                                       save_best_only=False, epoch_per=3,
                                       **save_kwargs)
...
if __name__ =='__main__':
    ...
    tf.app.flags.DEFINE_integer('batch_size', 256,'Number of training pairs per iteration.')
    tf.app.flags.DEFINE_integer('epochs',10,'number of training')
    tf.app.flags.DEFINE_integer('early_stopping_rounds',10,'number of early stopping')
    tf.app.flags.DEFINE_integer('num_classes',10,'class number of mnist data')
