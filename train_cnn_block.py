...
from pytfeager.model.cnn_block import CNN
...
from pytfeager.callbacks.model_callbacks import ReduceLROnPlateau
from pytfeager.config import cnn_block_config as config
...
def main(_):
    ...
    # 学习率变化
    lr_schedule = ReduceLROnPlateau(init_lr=learning_rate,factor=0.1,
                                    verbose=1, patience=4,mode='min',
                                    min_delta=1e-4, cooldown=0, min_lr=0.00001)
    optimizer = tf.train.AdamOptimizer(learning_rate=lr_schedule.learning_rate)
    # 初始化模型
