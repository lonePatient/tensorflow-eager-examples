...
from pytfeager.utils.loss_utils import mse_loss
from pytfeager.utils.eval_utils import r2
...
class Regressor(tf.keras.Model):
    ...
    def compute_loss(self,X,y):
        y_pred = self(X)[:,0]
        loss = mse_loss(y_pred=y_pred,y_true = y)
        return y_pred, loss
    ...
    def compute_score(self, y_pred, y_true):
        score = r2(y_pred=y_pred,y_true=y_true)
        return score
    ...
    def fit(self,trainDataset,valDataset,epochs,optimizer,ModelCheckPoint,progressbar,
        ...
        for i in range(epochs):
            ...
            write_summary.on_epoch_end(tensor=np.mean(score_list), name='train_r2', step=i)
            write_summary.on_epoch_end(tensor=val_score, name='val_r2', step=i)
