...
from pytfeager.model.mlp import MLP
...
from pytfeager.config import mlp_config as config
...
def main(_):
    ...
    # 初始化模型
    model = MLP(hidden_units=FLAGS.hidden_units,num_classes = FLAGS.num_classes)
    save_kwargs = {
        'model': model,
        'optimizer': optimizer,
    ...
    model_checkpoint = ModelCheckpoint(checkpoint_dir=checkpoint_dir, mode='min',monitor='val_los', save_best_only=False, epoch_per=3,
                                       **save_kwargs)
...
if __name__ =='__main__':
    ...
    tf.app.flags.DEFINE_integer('hidden_units',500,
